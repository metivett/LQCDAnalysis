...
namespace LQCDA {
    ...
    {
    public:
	virtual double operator() (const std::vector<double>& x, const std::vector<double>& params) =0;
    };
	...
	double eval(size_t k, const std::vector<double>& x, const std::vector<double>& params)
	    {
		if(k >= _m_fcts.size())
		    throw OutOfRange("FitModel::eval()", k);
		else return _m_fcts[k](x, params);
	    }
	std::vector<double> eval(const std::vector<double>& x, const std::vector<double>& params)
	    {
