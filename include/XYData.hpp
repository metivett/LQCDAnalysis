...
#define XY_DATA_HPP
 ...
 namespace LQCDA {
 	...
 	typename XYData<T>::block_t XYData<T>::x(std::initializer_list<index_t> l1, std::initializer_list<index_t> l2)
 	{
 		...
 		return _x.block(r2[0], r1[0], r2[1] - r2[0], r1[1] - r1[0]);
 	}
 	template<typename T>
 	...
 	{
 		auto r2 = check_range(l2, _xDim);
 		return _x.block(r2[0], i, r2[1] - r2[0], 1);
 	}
 	template<typename T>
 	...
 	{
 		auto r1 = check_range(l1, _nPts);
 		return _x.block(k, r1[0], 1, r1[1] - r1[0]);
 	}
 	template<typename T>
 	...
 	typename XYData<T>::block_t XYData<T>::y(std::initializer_list<index_t> l1, std::initializer_list<index_t> l2)
 	{
 		...
 		return _y.block(r2[0], r1[0], r2[1] - r2[0], r1[1] - r1[0]);
 	}
 	template<typename T>
 	...
 	{
 		auto r2 = check_range(l2, _yDim);
 		return _y.block(r2[0], i, r2[1] - r2[0], 1);
 	}
 	template<typename T>
 	...
 	{
 		auto r1 = check_range(l1, _nPts);
 		return _y.block(k, r1[0], 1, r1[1] - r1[0]);
 	}
 	template<typename T>
