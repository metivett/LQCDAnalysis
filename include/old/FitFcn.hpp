...
#define FIT_FCN_HPP_
 ...
 namespace LQCDA {
 	...
 	template<enum FitMethod M, typename T, int XDIM, int YDIM, unsigned int NPAR>
 	class FitFcn;

    template<typename T, int XDIM, int YDIM, unsigned int NPAR>
 	class FitFcn<FitMethod::LS, T, XDIM, YDIM, NPAR>
 	{
 	private:
 		static_assert(std::is_floating_point<T>::value, "Scalar type must be floating point");

 	protected:
 		typedef typename LQCDA::ParametrizedFunction<T, XDIM, YDIM, NPAR> Model;
 		typedef typename LQCDA::Point<T, XDIM, YDIM> Point;
 		typedef FitOptions<FitMethod::LS, T, XDIM, YDIM> FitOpts;

 		const DataSet<T, XDIM, YDIM> * _Data;
 		Model * _Model;
 		const FitOpts * _Options;

 		Vector<T, Dynamic> _Y;
 		mutable Vector<T, Dynamic> _MX;

 	public:
 		FitFcn(const DataSet<T, XDIM, YDIM> * data, Model * model, const FitOpts * opts)
 		: _Data(data), _Model(model), _Options(opts), _Y(data->Size() * YDIM), _MX(data->Size() * YDIM) 
 		{
 			// Fill _Y
 		...
 		double operator() (const std::vector<double>& params) const {
 			...
 			for(int p=0; p<NPAR; p++)
 				_Model->SetParamValue(p, params[p]);
 			// Fill _MX
 			for(int i=0; i<_Data->Size(); i++)
...
//  	class Chi2Fcn: public FitFcn<XT, YT>
//  	{
//  	private:
//  		const std::vector<YT> * _YWeights;
//  		const std::vector<XT> * _XWeights;

//  	public:
//  		Chi2Fcn(const std::vector<Point<XT, YT>> * data, ParametrizedFunction<XT, YT> * model)
//  		: FitFcn<XT, YT>(data, model) {}

//  		void SetYWeights(const std::vector<YT> * w) { _YWeights = w; }
//  		void SetXWeights(const std::vector<XT> * w) { _XWeights = w; }
//  		void SetYCorrelationMatrix() {}

//  		bool HasYWeights() const { return _YWeights !=0; }
//  		bool HasXWeights() const { return _XWeights !=0; }
//  		bool HasYCorrelation() const { return false; }
