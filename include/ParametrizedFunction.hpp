 ...
 ******************************************************************************/
    ...
    public:
        ...
        T getParValue(unsigned int i) const { checkParIndex(i); return _P[i]; }
        const std::vector<T> getParValues() const { return _P; }

        // Setters
        void setParameter(unsigned int i, T p) { checkParIndex(i); _P[i] = p; }
        void setParameters(const std::vector<T>& p) { checkNpar(p.size()); _P = p; }
